...
def rollout(board, current_player, deadline):
    ...
    """
    size  = board.size
    ADJ, TOP_CELLS, BOTTOM_START = _get_rollout_tables(size)   # ← add this
    ...
    while head < len(queue):
        ...
        if idx >= BOTTOM_START:           # pre-computed: (size-1)*size
            return RED
...
def choose_move(size, board, my_color, time_limit):
    ...
    return divmod(random.choice(board.empty_cells), size)
